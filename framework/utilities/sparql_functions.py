...
def get_sparql_results_expanded_wikidata(s_uri):  # , p_uri, o_uri):
    sparql_wd = SPARQLWrapper("https://query.wikidata.org/sparql")
    ...
    # Perform the SPARQL query
    sparql_result_p = execute_sparql_query(sparql_query, sparql_wd)
    results_bindings = sparql_result_p["results"]["bindings"]
    print(sparql_query)

    uri_label_pairs = [(result_bind["predicate"]["value"], result_bind["propLabel"]["value"],
                        result_bind["object"]["value"], result_bind["objectLabel"]["value"])
                       for result_bind in results_bindings]
